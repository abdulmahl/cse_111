...
def print_death_age(people_dict):
    ...
    for key, person_list in people_dict.items():
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for key, marriage_list in marriages_dict.items():
...
# Add code to the print_death_age function 
# that prints the birth year and death year 
# for each person.

def print_death_age(people_dict):
    for key, person_list in people_dict.items():
        name = person_list[NAME_INDEX]
        born = person_list[BIRTH_YEAR_INDEX]
        died = person_list[DEATH_YEAR_INDEX]

        print(f"{name}, Born {born}, Died {died}")

# Add to your program a function named 
# count_marriages that counts and prints the number
# of marriages that each person had in his or her lifetime. 
