...
def main():
    ...
    for inner_list in symbol_quantity_list:
        atoms = inner_list[QUANTITY_INDEX]

    total_molar_mass = 0
    for key, inner_list in periodic_table_dict.items():
        atoms = inner_list[QUANTITY_INDEX]
        atomic_mass = inner_list[ATOMIC_MASS_INDEX]


        total_molar_mass = atomic_mass * atoms
    # total_molar_mass += product
    print(f"{total_molar_mass:.3f} grams/mole")

    moles = mass_grams / total_molar_mass
